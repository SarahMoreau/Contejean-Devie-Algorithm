...
def memoire_premier_tour(valeur, nombre_equations, vecteur):
  ...
  from equation_homogene_dichotomie import dichotomie
  memoire = ["".join([str(_) for _ in valeur[0]])]
  liste = []
  for i in range(1,len(valeur),1):
        valeur_1 = "".join([str(_) for _ in valeur[i]])
        if valeur_1 in memoire:
            liste.append(i)
        else :
            memoire.append(valeur_1)
  while liste != []:
        del(valeur[liste[0]])
        del(vecteur[liste[0]])
        del (liste[0])
...
def memoire_tour_suivant(memoire, valeur_bis, nombre_equations, vecteurs_bis):
  ...
  for i in range(0, len(valeur_bis),1):
    valeur = "".join([str(_) for _ in valeur_bis[i]])
    if dichotomie(memoire,valeur)==True:
      liste.append(i)
    else :
